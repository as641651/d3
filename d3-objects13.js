...
const t = d3.transition().duration(1000);
    ...
    const update = (data) => {
        ...
        rects.enter()
            .append('rect')
            //.attr('width',0) //starting position // not needed as it is speicified in the tween
            ...
            .transition(t) // use a const
                .attrTween('width', widthTween) // position of width (both start and end are specified in tween)
...
const widthTween = (d) => { // takes in data as input

    // let : a variable whose scope is within the block
    let i = d3.interpolate(0, x.bandwidth()); // intepolate: start, end. 
    // i is a function
    //i(0): starting position
    //i(1): end position
    //i(0.5): half war between start and end position

    return function(t){
        //returns a functions that takes in a time ticker.
        // takes duration in ms as ticker. but its between 0 and 1
        return i(t);
    }
}
...
// transition is a specific tween

// tween is used for coomplex transitions
